 ...
 */
function getGapSignificance(minutes) {
    if (minutes < 5)
        return 'just_now';
    if (minutes < 60)
 ...
 */
function calculateSurfacingProbability(gapMinutes, timeOfDay, hasPendingEvents, alreadySurfacedThisSession) {
 ...
 */
function generateFeltSuggestion(context) {
    if (!context.shouldSurface)
        return undefined;
 ...
 */
export async function recordSessionTimestamp(userId) {
    const timestamp = new Date().toISOString();
    try {
        // Store a lightweight "ping" message to mark session activity
        await storeConversation(userId, `[SESSION_TIMESTAMP] Session active at ${timestamp}`, `[SESSION_ACK] Timestamp recorded: ${timestamp}`, { sessionName: `session-timestamp-${Date.now()}`, debug: false });
    }
    catch (e) {
 ...
 */
async function getLastSessionTimestamp(userId) {
    try {
        const result = await searchMemories(userId, 'SESSION_TIMESTAMP session active', { maxFacts: 5, debug: false });
 ...
 */
export async function recordMentionedEvent(userId, event, estimatedDate) {
    ...
    try {
        ...
        await storeConversation(userId, `[PENDING_EVENT] ${event}${estimatedDate ? ` (expected: ${estimatedDate})` : ''}`, `[EVENT_RECORDED] Tracking: ${JSON.stringify(eventData)}`, { sessionName: `event-${Date.now()}`, debug: false });
    }
    catch (e) {
 ...
 */
async function getPendingEvents(userId) {
    try {
        const result = await searchMemories(userId, 'PENDING_EVENT EVENT_RECORDED tracking', { maxFacts: 10, debug: false });
        const events = [];
        const now = new Date();
        for (const fact of result.facts) {
            try {
 ...
 */
export async function calculateTemporalContext(userId, alreadySurfacedThisSession = false) {
    ...
    // Calculate surfacing
    const { shouldSurface, surfaceType } = calculateSurfacingProbability(gapMinutes, timeOfDay, hasPendingEvents, alreadySurfacedThisSession);
    const context = {
        gap,
        timeOfDay,
        dayOfWeek,
 ...
 */
export function formatTemporalForMindstate(temporal) {
    const parts = [];
