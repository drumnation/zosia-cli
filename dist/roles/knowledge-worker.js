...
export class KnowledgeWorker {
     ...
     */
    async fetchSourcesByFrequency(frequency) {
        console.log(`[KnowledgeWorker] Fetching ${frequency} sources...`);
        for (const role of this.roles) {
            const sources = role.newsSources.filter((source) => source.updateFrequency === frequency);
     ...
     */
    async fetchSource(roleId, source) {
        try {
     ...
     */
    scheduleJob(frequency, schedule) {
     ...
     */
    defaultErrorHandler(roleId, sourceId, error) {
        console.error(`[KnowledgeWorker] Error fetching ${sourceId} for role ${roleId}:`, error.message);
    }
}
/**
