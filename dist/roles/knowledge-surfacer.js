 ...
 */
function calculateFreshness(dates) {
    if (dates.length === 0) {
        return 'none';
    }
    const now = Date.now();
    const hasRecent = dates.some((date) => now - date.getTime() < FRESHNESS_THRESHOLDS.recent);
    if (hasRecent) {
        return 'recent';
    }
    const hasStale = dates.some((date) => now - date.getTime() < FRESHNESS_THRESHOLDS.stale);
    if (hasStale) {
        return 'stale';
    }
    return 'none';
 ...
 */
async function surfaceRoleKnowledge(roleId, options = {}) {
 ...
 */
export async function surfaceKnowledgeForRoles(detectedRoles, options = {}) {
    const { confidenceThreshold = DEFAULT_CONFIDENCE_THRESHOLD, maxInsightsPerRole = DEFAULT_MAX_INSIGHTS, query, } = options;
    // Filter roles by confidence threshold
    const qualifyingRoles = detectedRoles.filter((role) => role.confidence >= confidenceThreshold);
    if (qualifyingRoles.length === 0) {
        return [];
    }
    console.log(`[KnowledgeSurfacer] Surfacing knowledge for ${qualifyingRoles.length} roles: ${qualifyingRoles.map((r) => r.roleId).join(', ')}`);
    // Query all roles in parallel for speed
    const surfacingPromises = qualifyingRoles.map((role) => surfaceRoleKnowledge(role.roleId, {
 ...
 */
export function toRoleKnowledgeState(surfacedKnowledge) {
    const result = {};
 ...
 */
function calculateFreshnessFromAges(ages) {
    if (ages.length === 0) {
        return 'none';
    }
    // Check for recent (less than 24h)
    const recentIndicators = ['just now', 'hour ago', 'hours ago'];
    const hasRecent = ages.some((age) => recentIndicators.some((indicator) => age.includes(indicator)));
    if (hasRecent) {
        return 'recent';
    }
    // Check for stale (less than 7 days)
    const staleIndicators = ['yesterday', 'days ago', 'week'];
    const hasStale = ages.some((age) => staleIndicators.some((indicator) => age.includes(indicator)));
    if (hasStale) {
        return 'stale';
    }
    return 'none';
 ...
 */
export function generateRoleIntelligencePrompt(roleKnowledge) {
    const roleIds = Object.keys(roleKnowledge);
    if (roleIds.length === 0) {
        return '';
    }
    const sections = ['## Current Role Intelligence\n'];
    for (const roleId of roleIds) {
        ...
        const freshnessLabel = state.freshness === 'recent'
            ? 'fresh'
 ...
 */
export async function surfaceAndGeneratePrompt(detectedRoles, options = {}) {
