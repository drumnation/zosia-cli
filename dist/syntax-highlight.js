...
export function parseCodeBlocks(markdown) {
    ...
    while ((inlineMatch = inlineRegex.exec(markdown)) !== null) {
        ...
        const isInsideFencedBlock = blocks.some((block) => inlineMatch.index >= block.startIndex &&
 ...
 */
export function formatCodeBlock(block) {
 ...
 */
export function renderWithHighlighting(markdown) {
    const { blocks, segments } = parseCodeBlocks(markdown);
    return segments
        .map((segment) => {
        if (segment.type === 'text') {
            // Highlight inline code in text
            return segment.content.replace(/`([^`]+)`/g, (_, code) => chalk.cyan.dim('`') + chalk.cyan(code) + chalk.cyan.dim('`'));
        }
        else {
