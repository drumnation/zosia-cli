 ...
 * ```
 */
export function makeOuraFetcher(config) {
...
// Data Transformers
// ============================================================================
function transformSleepData(raw) {
    if (!raw) {
        return {
            score: 0,
            duration: 0,
...
function transformReadinessData(raw) {
    if (!raw) {
        return {
            score: 0,
            hrv: 0,
...
function transformActivityData(raw) {
    if (!raw) {
        return {
            score: 0,
            steps: 0,
...
function detectPatterns(sleepHistory, readinessHistory) {
    // Analyze sleep trend
    ...
    if (readinessHistory.length >= 3) {
        ...
        if (avgRecent > avgPrevious + 3)
            hrvTrend = 'up';
        else if (avgRecent < avgPrevious - 3)
            hrvTrend = 'down';
    }
