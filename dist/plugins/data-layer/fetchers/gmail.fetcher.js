 ...
 * ```
 */
export function makeGmailFetcher(config) {
    ...
    return async () => {
        ...
        try {
            ...
            const [messagesResponse, labelsResponse] = await Promise.all([
                fetch(`${GMAIL_API_BASE}/users/${userId}/messages?maxResults=${maxResults}&labelIds=INBOX`, {
                    headers: {
            ...
            if (!messagesResponse.ok || !labelsResponse.ok) {
                throw new Error(`Gmail API error: messages=${messagesResponse.status}, labels=${labelsResponse.status}`);
            }
            ...
            const messageDetails = await Promise.all((messagesData.messages || []).slice(0, maxResults).map(async (msg) => {
                const detailResponse = await fetch(`${GMAIL_API_BASE}/users/${userId}/messages/${msg.id}?format=metadata&metadataHeaders=Subject&metadataHeaders=From&metadataHeaders=Date`, {
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
                    },
                });
...
function detectPatterns(messages, unreadCount) {
    // Calculate email volume
    let emailVolume = 'normal';
    if (unreadCount > 50)
        emailVolume = 'high';
    ...
    const importantUnread = messages.filter((m) => m.isUnread && m.labels.includes('IMPORTANT')).length;
