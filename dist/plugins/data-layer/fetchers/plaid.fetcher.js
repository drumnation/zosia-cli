...
function simplifyCategory(plaidCategories) {
 ...
 * ```
 */
export function makePlaidFetcher(config) {
    ...
    return async () => {
        ...
        try {
            ...
            const recentTransactions = transformTransactions(transactionsData?.transactions || []);
            const patterns = analyzePatterns(transactionsData?.transactions || [], balances);
            if (debug) {
                console.log(`[PlaidFetcher] Checking: $${balances.checking}, ` +
...
// Data Transformers
// ============================================================================
function aggregateBalances(accounts) {
    let checking = 0;
    let savings = 0;
    let credit = 0;
...
function analyzePatterns(transactions, balances) {
