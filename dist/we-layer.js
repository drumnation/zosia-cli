...
import { checkGraphitiHealth, searchMemories, factsToAssociations, storeConversation, } from './graphiti-memory.js';
import { getUnconsciousSpawner, } from './unconscious-spawner.js';
 ...
 */
export function refreshClaudeCodeStatus() {
 ...
 */
export function getClaudeCodeStatusSummary() {
    const status = detectClaudeCodeAuth();
...
let unconsciousConfig = {};
/**
 * Initialize We-layer - check Claude Code auth and Graphiti health
 */
export async function initWeLayerPool(options = false) {
 ...
 */
export async function shutdownWeLayerPool() {
    // Nothing to shutdown in V1
}
/**
 * We-layer retrieval - Uses Graphiti with pattern fallback
 */
export async function retrieveDeeper(userId, message, existingAssociations = []) {
 ...
 */
function runPatternMatching(message, debug) {
    const associations = [];
    ...
    if (lowerMessage.includes('meaningful') || lowerMessage.includes('purpose') || lowerMessage.includes('meaning')) {
        ...
        if (debug)
            console.log(`│   ✓ Pattern: meaning/purpose → HUNCH|medium`);
    }
    ...
    if (lowerMessage.includes('project') || lowerMessage.includes('build') || lowerMessage.includes('create')) {
        ...
        if (debug)
            console.log(`│   ✓ Pattern: project/build → PULL|faint`);
    }
    ...
    if (lowerMessage.includes('dave') || lowerMessage.includes('ben')) {
        ...
        if (debug)
            console.log(`│   ✓ Pattern: identity (${name}) → RECALL|strong`);
    }
    ...
    if (lowerMessage.includes('frustrat') || lowerMessage.includes('stuck') || lowerMessage.includes('hard') || lowerMessage.includes('difficult')) {
        ...
        if (debug)
            console.log(`│   ✓ Pattern: struggle → SIGN|medium`);
    }
    ...
    if (lowerMessage.includes('you') && (lowerMessage.includes('think') || lowerMessage.includes('feel') || lowerMessage.includes('remember'))) {
        ...
        if (debug)
            console.log(`│   ✓ Pattern: inner experience → PULL|medium`);
    }
    return associations;
}
/**
 ...
 */
export async function persistConversation(userId, userMessage, assistantResponse, debug = false) {
    if (!graphitiHealthy) {
        if (debug) {
 ...
 */
export async function getGraphitiStatusAsync() {
 ...
 */
export function getGraphitiStatus() {
    return { healthy: graphitiHealthy, latencyMs: graphitiLatencyMs };
}
/**
 * FUTURE: The General - Root orchestrator
 */
export async function theGeneral(userId, message, _context) {
    console.log(`[The General] Dispatching for user ${userId}:`, message.slice(0, 50));
}
/**
 * FUTURE: Memory Commander
 */
export async function memoryCommander(userId, message) {
 ...
 */
export async function runDeepUnconscious(userId, sessionId, message, debug = false) {
    const startTime = Date.now();
 ...
 */
export function isDeepUnconsciousEnabled() {
    return deepUnconsciousEnabled;
}
/**
 * Enable/disable deep unconscious mode at runtime
 */
export function setDeepUnconsciousMode(enabled, config) {
    const status = detectClaudeCodeAuth();
