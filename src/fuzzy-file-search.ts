 ...
 */
export async function scanFiles(cwd: string = process.cwd()): Promise<FileEntry[]> {
  // Check cache
  const now = Date.now();
  ...
  try {
    ...
    const entries: FileEntry[] = files.slice(0, 5000).map((filePath) => {
 ...
 */
export function detectAtSearch(input: string, cursorPosition?: number): FuzzySearchState | null {
  if (!input.includes('@')) {
    return null;
  }
 ...
 */
export function formatFileEntry(file: FileEntry, maxWidth: number = 50): string {
