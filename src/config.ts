...
// Local type definitions (previously from @dev-garden/shared-types)
...
let resetEmbeddingServiceFn: (() => void) | null = null;

async function getResetEmbeddingService(): Promise<() => void> {
...
export interface CostTracking {
  ...
  embeddingsByProvider: {
    jina: { tokens: number; cost: number };
    openai: { tokens: number; cost: number };
  };
  // LLM costs (conscious mind)
  totalLlmInputTokens: number;
  totalLlmOutputTokens: number;
  totalLlmCostUsd: number;
  llmCostsByModel: Record<string, { inputTokens: number; outputTokens: number; cost: number }>;
  // Tracking
  lastReset: string;
  sessionCount: number;
}

// Conscious mind model settings
export interface ConsciousMindConfig {
  model: string;               // e.g., 'google/gemma-2-9b-it'
...
export interface CustomPrompts {
  ...
  subconscious?: string;       // Override We-Layer prompt
  identityKernel?: string;     // Override identity kernel
  handoff?: string;            // Override context handoff prompt
}

// Context handoff settings
export interface HandoffSettings {
  prompt?: string;             // Custom handoff prompt (undefined = use default)
  threshold: number;           // Context % that triggers handoff (0-100)
  enabled: boolean;            // Whether handoff is enabled
}

// Full config interface
export interface ConfigData {
  // === Embedding Config (existing) ===
...
export function loadConfig(): ConfigData {
  ...
  try {
    ...
    const stored = JSON.parse(data) as Partial<ConfigData>;
...
export function getPreferredProvider(): EmbeddingProviderType {
  const config = loadConfig();
  return config.preferredEmbeddingProvider;
}

export async function setPreferredProvider(provider: EmbeddingProviderType): Promise<void> {
  const config = loadConfig();
...
export function getEmbeddingServiceConfig(): EmbeddingServiceConfig {
  const config = loadConfig();
...
export function getConfigSummary(): string {
  ...
  if (config.customPrompts?.conscious || config.customPrompts?.subconscious) {
    ...
    if (config.customPrompts.conscious) lines.push('• Custom conscious prompt set');
    if (config.customPrompts.subconscious) lines.push('• Custom subconscious prompt set');
    if (config.customPrompts.identityKernel) lines.push('• Custom identity kernel set');
  }
...
export async function setOpenRouterKey(apiKey: string): Promise<boolean> {
...
export function getConsciousMindConfig(): ConsciousMindConfig {
  const config = loadConfig();
  return config.consciousMind;
}

export async function setConsciousMindModel(model: string): Promise<void> {
  const config = loadConfig();
...
export async function setConsciousMindTemperature(temperature: number): Promise<void> {
...
export async function setConsciousMindMaxTokens(maxTokens: number): Promise<void> {
...
export async function setCustomPrompt(type: 'conscious' | 'subconscious' | 'identityKernel', prompt: string): Promise<void> {
  const config = loadConfig();
...
export async function clearCustomPrompt(type: 'conscious' | 'subconscious' | 'identityKernel'): Promise<void> {
  const config = loadConfig();
  if (config.customPrompts) {
...
export function getHandoffSettings(): HandoffSettings {
  const config = loadConfig();
...
export async function clearHandoffPrompt(): Promise<void> {
  const config = loadConfig();
...
export function setCostTrackingEnabled(enabled: boolean): void {
  const config = loadConfig();
...
export function trackEmbeddingUsage(provider: 'jina' | 'openai', tokens: number, cost: number): void {
  const config = loadConfig();
...
export function trackLlmUsage(model: string, inputTokens: number, outputTokens: number, cost: number): void {
  const config = loadConfig();
  if (!config.costTracking.enabled) return;
