...
export function getFileSize(imagePath: string): string {
  try {
...
export function formatImageRef(imagePath: string): string {
...
export function displayImageKitty(imagePath: string, maxHeight: number = 10): boolean {
  if (!isKittyTerminal() || !existsSync(imagePath)) return false;
...
export function generateImageCard(imagePath: string): string[] {
  const filename = basename(imagePath);
  const dims = getImageDimensions(imagePath);
  const size = getFileSize(imagePath);
 ...
 */
export function createImageToken(imagePath: string): { display: string; actual: string } {
  const filename = basename(imagePath);
...
export function parseImageTokens(text: string): {
  cleanText: string;
  images: Array<{ display: string; path: string }>;
} {
  const imagePattern = /\[📷 ([^\]]+)\]\{([^}]+)\}/g;
  const images: Array<{ display: string; path: string }> = [];
...
export function encodeImageToken(imagePath: string): string {
  const filename = basename(imagePath);
...
export function decodeImageTokens(text: string): string {
  return text.replace(/\[📷 [^\]]+\]\{([^}]+)\}/g, '@$1');
}

/** Check if text contains image tokens */
export function hasImageTokens(text: string): boolean {
  return /\[📷 [^\]]+\]/.test(text);
}
