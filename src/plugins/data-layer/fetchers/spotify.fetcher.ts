...
// ============================================================================
// API Response Types (from Spotify Web API)
// ============================================================================
...
interface SpotifyApiAudioFeatures {
  id: string;
...
function isKidsMusic(trackName: string, artistName: string): boolean {
 ...
 * ```
 */
export function makeSpotifyFetcher(config: SpotifyFetcherConfig): () => Promise<SpotifyData> {
  const { accessToken, debug = false } = config;

  return async (): Promise<SpotifyData> => {
    if (debug) {
    ...
    try {
      ...
      const [recentlyPlayedResponse, currentlyPlayingResponse, topArtistsResponse] =
        await Promise.all([
      ...
      const recentlyPlayed = recentlyPlayedResponse.ok
        ? ((await recentlyPlayedResponse.json()) as { items: SpotifyApiPlayHistory[] })
        : { items: [] };

      const currentlyPlaying =
        currentlyPlayingResponse.ok && currentlyPlayingResponse.status !== 204
          ? ((await currentlyPlayingResponse.json()) as SpotifyApiCurrentlyPlaying)
          : null;

      const topArtists = topArtistsResponse.ok
        ? ((await topArtistsResponse.json()) as { items: SpotifyApiArtist[] })
        : { items: [] };
      ...
      let audioFeaturesData: SpotifyApiAudioFeatures[] = [];
      if (trackIds) {
        ...
        if (audioFeaturesResponse.ok) {
          const data = (await audioFeaturesResponse.json()) as {
      ...
      if (debug) {
        console.log(
          `[SpotifyFetcher] Valence: ${aggregatedAudioFeatures.valence.toFixed(2)}, ` +
...
function transformRecentlyPlayed(items: SpotifyApiPlayHistory[]): SpotifyTrack[] {
...
function transformCurrentlyPlaying(
  data: SpotifyApiCurrentlyPlaying | null
): SpotifyCurrentlyPlaying | null {
  if (!data || !data.item) {
    return null;
  }
...
function transformTopArtists(items: SpotifyApiArtist[]): { name: string; genres: string[] }[] {
...
function aggregateAudioFeatures(features: SpotifyApiAudioFeatures[]): SpotifyAudioFeatures {
...
function detectPatterns(
  recentlyPlayed: SpotifyApiPlayHistory[],
  audioFeatures: SpotifyApiAudioFeatures[],
  topArtists: SpotifyApiArtist[]
): SpotifyPatterns {
  // Detect mood trend from valence
  let moodTrend: 'upbeat' | 'mellow' | 'mixed' = 'mixed';
  if (audioFeatures.length >= 3) {
    const avgValence = audioFeatures.reduce((sum, f) => sum + f.valence, 0) / audioFeatures.length;
    if (avgValence > 0.6) moodTrend = 'upbeat';
    else if (avgValence < 0.4) moodTrend = 'mellow';
  }
  ...
  if (avgHour >= 22 || avgHour < 4) listeningTime = 'late night';
  else if (avgHour >= 4 && avgHour < 9) listeningTime = 'early morning';
  else if (avgHour >= 9 && avgHour < 12) listeningTime = 'morning';
  else if (avgHour >= 17 && avgHour < 22) listeningTime = 'evening';
  ...
  const kidsCount = recentlyPlayed.filter((item) =>
    isKidsMusic(item.track.name, item.track.artists[0]?.name || '')
  ).length;
...
export function makeMockSpotifyFetcher(options?: {
  ...
  isKidsMusic?: boolean;
  currentlyPlaying?: { track: string; artist: string } | null;
}): () => Promise<SpotifyData> {
  return async (): Promise<SpotifyData> => {
    // Simulate API delay
