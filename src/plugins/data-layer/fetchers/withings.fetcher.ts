...
import type { WithingsData, WithingsWeight, WithingsPatterns } from '../types.js';
...
// ============================================================================
// API Response Types (from Withings API)
// ============================================================================

interface WithingsApiMeasureGroup {
  grpid: number;
  attrib: number;
  date: number;
  created: number;
...
interface WithingsApiMeasureResponse {
  status: number;
  body: {
    updatetime: number;
    timezone: string;
    measuregrps: WithingsApiMeasureGroup[];
  };
}
 ...
 * ```
 */
export function makeWithingsFetcher(
  config: WithingsFetcherConfig
): () => Promise<WithingsData> {
  const { accessToken, debug = false } = config;

  return async (): Promise<WithingsData> => {
    if (debug) {
    ...
    try {
      ...
      const data = (await response.json()) as WithingsApiMeasureResponse;
      ...
      if (debug) {
        console.log(
          `[WithingsFetcher] Latest weight: ${latestMeasurement?.weight}kg, Trend: ${patterns.weightTrend}`
        );
      }
...
function transformMeasurements(groups: WithingsApiMeasureGroup[]): WithingsWeight[] {
  // Sort by date descending (most recent first)
  const sortedGroups = [...groups].sort((a, b) => b.date - a.date);

  return sortedGroups.map((group) => {
    const getMeasure = (type: number): number | null => {
...
function detectPatterns(measurements: WithingsWeight[]): WithingsPatterns {
  ...
  let weightTrend: 'gaining' | 'stable' | 'losing' = 'stable';
  if (measurements.length >= 4) {
    ...
    const avgRecent =
      recent.reduce((sum, m) => sum + m.weight, 0) / recent.length;
  ...
  let measurementFrequency: 'daily' | 'regular' | 'sporadic' = 'sporadic';
  if (measurements.length >= 7) {
    ...
    if (avgDaysBetween <= 1.5) measurementFrequency = 'daily';
    else if (avgDaysBetween <= 4) measurementFrequency = 'regular';
  }
...
export function makeMockWithingsFetcher(options?: {
  ...
  return async (): Promise<WithingsData> => {
    ...
    const mockMeasurements: WithingsWeight[] = [];
    for (let i = 0; i < 14; i++) {
