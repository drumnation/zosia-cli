...
interface CalendarFetcherConfig {
  ...
  custodyCalendarId?: string; // Separate calendar for custody schedule
  debug?: boolean;
}

// ============================================================================
// API Response Types (from Google Calendar API)
// ============================================================================
...
function categorizeEvent(event: GoogleCalendarEvent): CalendarEvent['type'] {
 ...
 * ```
 */
export function makeCalendarFetcher(
  ...
  return async (): Promise<CalendarData> => {
    if (debug) {
    ...
    try {
      ...
      const eventsData = (await eventsResponse.json()) as GoogleCalendarListResponse;

      // Optionally fetch custody calendar if specified
      let custodyEvents: GoogleCalendarEvent[] = [];
      if (custodyCalendarId) {
        const custodyResponse = await fetch(
          `${GOOGLE_CALENDAR_API_BASE}/calendars/${encodeURIComponent(custodyCalendarId)}/events?` +
            new URLSearchParams({
              timeMin: todayStart.toISOString(),
              timeMax: weekEnd.toISOString(),
              singleEvents: 'true',
        ...
        if (custodyResponse.ok) {
          const custodyData = (await custodyResponse.json()) as GoogleCalendarListResponse;
      ...
      const todayEvents = transformedEvents.filter(
        (e) => e.start >= todayStart && e.start <= todayEnd
      );
      ...
      if (debug) {
        console.log(
          `[CalendarFetcher] Today: ${todayEvents.length} events, ` +
...
function detectCustodyStatus(
  ...
  if (custodyEvents.length === 0) {
    const kidsEvents = events.filter(
      (e) => categorizeEvent(e) === 'kids' || categorizeEvent(e) === 'custody'
    );
...
function analyzeCalendarPatterns(todayEvents: CalendarEvent[]): CalendarPatterns {
  // Count meeting hours
  let meetingHours = 0;
  ...
  let busyLevel: 'light' | 'moderate' | 'heavy' = 'moderate';
      ...
      upcoming: [
        {
          ...
          type: isWeekOn ? 'kids' : ('personal' as const),
        },
      ],
      custody: {
        isWeekOn,
        transitionDate: new Date(
          Date.now() + (options?.daysUntilTransition ?? 3) * 24 * 60 * 60 * 1000
        ),
