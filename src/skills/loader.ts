 ...
 */
function parseSkillMd(content: string): { frontmatter: SkillManifest; body: string } {
  const frontmatterMatch = content.match(/^---\n([\s\S]*?)\n---\n([\s\S]*)$/);
  if (!frontmatterMatch) {
    throw new Error('SKILL.md must have YAML frontmatter');
  }

  const frontmatter = yaml.load(frontmatterMatch[1]) as SkillManifest;
...
export async function loadSkill(skillId: string): Promise<Skill | null> {
  ...
  let feltVocabulary: FeltVocabulary | undefined;
  ...
  if (fs.existsSync(feltPath)) {
    try {
      ...
      feltVocabulary = feltModule.default ||
        ...
        Object.values(feltModule).find(v =>
          typeof v === 'object' && v !== null && 'subtle' in (Object.values(v as object)[0] || {})
        );
    } catch (e) {
 ...
 */
function sortByDependencies(skills: Skill[]): Skill[] {
 ...
 */
export async function composeSkillPrompts(
  baseILayerPrompt: string,
  baseWeLayerPrompt: string,
  enabledSkillIds?: string[]
  ...
  const feltVocabulary: FeltVocabulary = {};
  for (const skill of skills) {
