 ...
 */
function buildGroupIds(userId: string): string[] {
  return [
 ...
 */
export async function checkGraphitiHealth(): Promise<{ healthy: boolean; latencyMs: number }> {
...
export async function searchMemories(
  userId: string,
  query: string,
  options: { maxFacts?: number; debug?: boolean; useLocalEmbedding?: boolean } = {}
): Promise<{ facts: SearchResult['facts']; latencyMs: number; embeddingProvider?: string }> {
  const start = Date.now();
 ...
 */
export function factsToAssociations(facts: SearchResult['facts']): Association[] {
  return facts.map((fact) => {
    // Determine association type based on fact content
    let type: Association['type'] = 'RECALL';
    let intensity: Association['intensity'] = 'medium';
...
export async function storeConversation(
  ...
  options: { sessionName?: string; debug?: boolean } = {}
 ...
 */
export async function storeTeaching(
  userId: string,
  topic: string,
  teaching: string,
