...
/** Persisted session format (includes version) */
...
function isValidSession(data: unknown): data is Session {
  ...
  if (typeof session.model !== 'string') return false;
  if (!Array.isArray(session.messages)) return false;
  if (typeof session.tokenUsage !== 'object') return false;
 ...
 */
export async function listSessions(directory: string): Promise<SessionMetadata[]> {
...
export function exportToMarkdown(
  ...
  for (const msg of session.messages) {
    ...
    if (opts.includeTokens && msg.role === 'assistant' && (msg.promptTokens || msg.completionTokens)) {
      ...
      if (msg.promptTokens) tokenInfo.push(`${msg.promptTokens} prompt`);
      if (msg.completionTokens) tokenInfo.push(`${msg.completionTokens} completion`);
  ...
  const exported = {
    ...
    messages: session.messages.map(msg => {
      ...
      if (opts.includeTokens && msg.role === 'assistant') {
        if (msg.promptTokens !== undefined) exported.promptTokens = msg.promptTokens;
        if (msg.completionTokens !== undefined) exported.completionTokens = msg.completionTokens;
      }
 ...
 */
export function generateSessionName(messages: SessionMessage[]): string {
